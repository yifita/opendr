...
class RigidTransform(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.v:
            ...
            JS = np.repeat(
                np.arange(self.v.r.size).reshape((-1, 3)), 3, axis=0)
...
class ProjectPoints(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.rt:
            ...
            IS = np.repeat(np.arange(self.v.r.size*2/3, dtype="int64"), 3)
            JS = np.asarray(np.repeat(np.arange(self.v.r.size, dtype="int64").reshape(
                (-1, 3)), 2, axis=0), order='C').ravel()
            result = sp.csc_matrix((data, (IS, JS)))
            return result

    def unproject_points(self, uvd, camera_space=False):
        cam = ProjectPoints3D(**{k: getattr(self, k)
                                 for k in self.dterms if hasattr(self, k)})

        try:
            xy_undistorted_camspace = cv2.undistortPoints(np.asarray(
                uvd[:, :2].reshape((1, -1, 2)).copy()), np.asarray(cam.camera_mtx), cam.k.r)
            xyz_camera_space = np.hstack(
                (xy_undistorted_camspace.squeeze(), col(uvd[:, 2])))
            # scale x,y by z
            xyz_camera_space[:, :2] *= col(xyz_camera_space[:, 2])
            if camera_space:
                return xyz_camera_space
            other_answer = xyz_camera_space - \
                row(cam.view_mtx[:, 3])  # translate
            ...
            ch.minimize(cam - uvd, x0=[cam.v],
                        method='dogleg', options={'disp': 0})
            result = cam.v.r
        return result
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if sp.issparse(result):
            ...
            result = sp.csc_matrix(
                (data, (IS, JS)), shape=(self.v.r.size, wrt.r.size))
        else:
